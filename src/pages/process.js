...
<h3>3. Re-frame the problem</h3>
    ...
    </p><p>So, while the feature request might have been “we need a feature that allows to transfer money to the user when the balance is low” (which is a request that already contains the solution), the problem that originated it might have been “transferring money to the user is time consuming and requires checking constantly the balance”. 
    <br></br>This new reframing of the problem opens up new paths towards the solution (either implement the scheduler, or an automatic alert when the balance is low).</p>
<img src="../images/process4.jpg"></img>
<h3>4. Design the solution</h3>
    <p>The problem is now identified and data is available to put it into the wider context of the product. It is now time to frame the solution as in “decide which one of these paths is going to the solution which is better fit for the problem”. <br></br>For this purpose it can be useful to put in the form of questions which features should the solution have - “should the user be able to set an automatic reminder?” “Should the user be able to import events?” - and build a list of possible solution approaches. </p>
    <p>The aim is to narrow down the options and form an assumption that will be tested with a prototype.
    Since the aim is testing the assumption, the prototype would ideally be focused around that, and making it assumption-centered, trying to strip it from all the frills and unnecessary details that might distract the user when testing.<br></br>At this step it is also ideal to chat with the developers and anyone else that will be involved in the process (QA, wider design team, customer care) to gather their insights on the solution from their perspective.</p>
<img src="../images/process5.jpg"></img>
<h3>5. Test the solution</h3>
    <p>Depending on the resources and time available, user testing is always both challenging and necessary. <br></br>Even if the resources are low and time is tight, testing a sample of users that is representative of the larger user base of the product is extremely important, even more so than having a large sample which is not representative and thus, biased (see the Literary Digest case in 1936).</p>
    <p>Collecting notes - or better yet record the audio - in the most exhaustive way means it is easier if the interviews are conducted with the help of a UX researcher (if possible, or at least another human being who can write) in order to keep both the quality of the notes and the interview high.</p>
<img src="../images/process6.jpg"></img>
<h3>6. Implement the solution</h3>
    <p>So now, is the assumption validated or not? If so, what are the pain points and strengths of the design? Assuming it all went well (assumption validated and minor pain points) the prototype will have to be turned into actual screens and requirements to give the developers.<br></br>In order to pave the path for future iterations, defining which are the KPIs and success indicators of the feature is a mandatory task that might require help from other team members (marketing people, BE and FE devs).</p>
    <p>In case the assumption was not tested it would be necessary to go back to the previous step of designing the solution or even reviewing the problem itself, and start again.<br></br>
...
<h3>7. Ship the feature</h3>
    <p>Well, this is self explanatory.<br></br>Just get it out there and let the world know it is out there.</p>
<img src="../images/process8.jpg"></img>
<h3>8. Follow the feature’s success</h3>
    <p>If everything was rightly done metrics should be available for gathering.<br></br>Checking in with customer care and give them the heads up about what is going on and setting a preferential channel for customer feedbacks regarding all aspects of the new feature is also a good idea to monitor the pulse of the situation.</p>
<img src="../images/process9.jpg"></img>
<h3>9. Did the solution solve the problem?</h3>
    <p>The feature has been shipped and available to the public for a few months now, and depending on the roadmap and other issues it should be time to ask the question: did the larger user base actually found the problem solved by the implemented solution?<br></br>
    In the perfect world, there are large parties and gathering being held to celebrate the simple brilliant beauty of the solution and world famine is now a memory just because of this feature’s problem solving abilities.</p>
    <p>In reality, not everyone is pleased (users and/or teammates), other issues have arisen and it might be time to solve niche problems or - regardless the entire process - we might have missed the mark.</p>
    <p>So, let’s keep faith in the process and start all over again (with more insight).</p>
